...
export default function Home() {
  ...
  const [imgNp, setImgNp] = useState<any>(null);
  const [heatmap, setHeatmap] = useState<any>(null);

  const [aiChoice, setAiChoice] = useState<"gemini" | "mistral" | "both">("gemini");
  ...
  return (
    <div className="bg-black min-h-screen text-white px-20 py-20">
      ...
      <div className="mt-10 mb-5">
        ...
        <p className="text-2xl w-400">
          You can upload a brain MRI image and the system will predict the dementia level of the uploaded image. It predicts by using a model Swin Transformer trained by Brain MRI dataset with four labels. After predicting, it will generates a Grad-CAM heatmap, and explains the result using Gemini or Mistral, or you can use both if you want to compare the explanation by the two LLMs!
        </p>
      </div>
